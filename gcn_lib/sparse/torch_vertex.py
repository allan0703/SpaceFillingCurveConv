...
class AvgPool(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        x_j = tg.utils.scatter_(self.aggr, torch.index_select(x, 0, edge_index[0]),
                                edge_index[1],
                                dim_size=x.shape[0])
...
class MRConv(nn.Module):
    ...
    def forward(self, x, edge_index):
        """"""
        x_j = tg.utils.scatter_(self.aggr,
                                torch.index_select(x, 0, edge_index[0]) - torch.index_select(x, 0, edge_index[1]),
