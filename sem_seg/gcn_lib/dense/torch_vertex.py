...
class AvgPool2d(nn.Module):
    ...
    def __init__(self, out_channels, act=None, norm=None):
    ...
    def forward(self, x, edge_index):
        ...
        x_j = self.aggr(x_j, dim=-1, keepdim=True)
        return self.unlinear(x_j)
...
class MaxPool2d(nn.Module):
    ...
    def forward(self, x, edge_index):
        x_j = batched_index_select(x, edge_index[0])
        x_j, _ = self.aggr(x_j, dim=-1, keepdim=True)
        return self.unlinear(x_j)
...
class GNAConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
                 act='relu', norm=None, bias=False, aggr='max',
                 negative_slope=0.2, dropout=0):
        super(GNAConv2d, self).__init__()
        ...
        self.dropout = dropout
        self.nn = BasicConv([in_channels * 2, out_channels], act, norm, bias)
        self.att = nn.Parameter(torch.Tensor(1, in_channels, 1, 1))
    ...
    def reset_parameters(self):
        nn.init.kaiming_normal_(self.att)

    def forward(self, x, edge_index):
        x_i = batched_index_select(x, edge_index[1])
        x_j = batched_index_select(x, edge_index[0])

        alpha = ((x_j - x_i) * self.att).sum(dim=1, keepdim=True)  # -1 xk
        alpha = F.leaky_relu(alpha, self.negative_slope)
        alpha = F.softmax(alpha, dim=-1)
        alpha = F.dropout(alpha, p=self.dropout, training=self.training)

        x = self.aggr(self.nn(torch.cat([(x_j - x_i) * alpha, x_i], dim=1)), -1, keepdim=True)[0]
        return x
