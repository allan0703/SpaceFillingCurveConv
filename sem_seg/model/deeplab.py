...
from model.resnet import resnet18, resnet101, resnet5
...
class DeepLab(nn.Module):
    def __init__(self, backbone='xception', input_size=3, output_stride=16, num_classes=21, kernel_size=9, sigma=1.0):
        ...
        elif backbone == 'resnet5':
            self.backbone = resnet5(input_size=input_size, kernel_size=kernel_size, sigma=sigma)
            sigma *= 2
            self.aspp = aspp(in_channels=128, out_channels=256, output_stride=output_stride,
                             kernel_size=kernel_size, sigma=sigma)
...
if __name__ == '__main__':
    ...
    net = deeplab(input_size=4, num_classes=13, backbone='resnet5', kernel_size=9)
